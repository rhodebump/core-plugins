...
function flatten($array) {
    $a = array();
    foreach ($array as $e) {
        if (is_array($e)) $a = array_merge($a, flatten($e));
        else $a[] = $e;
    }
    return $a;
}
...
class LDAPAuthentication {
     ...
     */
    static $schemas = array('msad' => array('user' => array('filter' => '(objectClass=user)', 'base' => 'CN=Users', 'first' => 'givenName', 'last' => 'sn', 'full' => 'displayName', 'email' => 'mail', 'phone' => 'telephoneNumber', 'mobile' => false, 'username' => 'sAMAccountName', 'dn' => '{username}@{domain}', 'search' => '(&(objectCategory=person)(objectClass=user)(|(sAMAccountName={q}*)(firstName={q}*)(lastName={q}*)(displayName={q}*)))', 'lookup' => '(&(objectCategory=person)(objectClass=user)({attr}={q}))',), 'group' => array('ismember' => '(&(objectClass=user)(sAMAccountName={username})
                    (|(memberOf={distinguishedName})(primaryGroupId={primaryGroupToken})))', 'lookup' => '(&(objectClass=group)(sAMAccountName={groupname}))',),),
    // A general approach for RFC-2307
    '2307' => array('user' => array('filter' => '(objectClass=inetOrgPerson)', 'first' => 'gn', 'last' => 'sn', 'full' => array('displayName', 'gecos', 'cn'), 'email' => 'mail', 'phone' => 'telephoneNumber', 'mobile' => 'mobileTelephoneNumber', 'username' => 'uid', 'dn' => 'uid={username},{search_base}', 'search' => '(&(objectClass=inetOrgPerson)(|(uid={q}*)(displayName={q}*)(cn={q}*)))', 'lookup' => '(&(objectClass=inetOrgPerson)({attr}={q}))',),),);
    var $config;
    var $type = 'staff';
    ...
    function getServers() {
        if (!($servers = $this->getConfig()->get('servers')) || !($servers = preg_split('/\s+/', $servers))) {
    ...
    function authenticate($username, $password = null) {
        ...
        if ($schema_type == 'msad') {
            ...
            // allows user@domain
            if (strpos($username, '\\') !== false) list($domain, $username) = explode('\\', $username);
            else $domain = $config->get('domain');
        }
        // Create the DN string for the bind based on the directory schema
        $dn = preg_replace_callback(':\{([^}]+)\}:', function ($match) use ($username, $domain, $config) {
            switch ($match[1]) {
                case 'username':
                    return $username;
                case 'domain':
                    return $domain;
                case 'search_base':
                    if (!$config->get('search_base')) return 'dc=' . implode(',dc=', explode('.', $config->get('domain')));
        ...
        if (!PEAR::isError($r)) return $this->lookupAndSync($username, $dn);
        // Another effort is to search for the user
        if (!$this->_bind($c)) return null;
        $r = $c->search($this->getSearchBase(), str_replace(array('{attr}', '{q}'),
        // Assume email address if the $username contains an @ sign
        array(strpos($username, '@') ? $schema['email'] : $schema['username'], $username), $schema['lookup']), array('sizelimit' => 1));
        if (PEAR::isError($r) || !$r->count()) return null;
        // Attempt to bind as the DN of the user looked up with the password
        // specified
        $bound = $c->bind($r->current()->dn(), $password);
        if (PEAR::isError($bound)) return null;
    ...
    function getSchema($connection) {
        ...
        if (!$schema || $schema == 'auto') {
            ...
            if (($caps = $dse->getValue('supportedCapabilities')) && in_array('1.2.840.113556.1.4.800', $caps)) {
                $this->getConfig()->set('schema', 'msad');
                return 'msad';
            }
        } elseif ($schema) return $schema;
        // Fallback
        return '2307';
    }
    function lookup($lookup_dn, $bind = true) {
        $c = $this->getConnection();
        if ($bind && !$this->_bind($c)) return null;
        $schema = static ::$schemas[$this->getSchema($c) ];
        $schema = $schema['user'];
        $opts = array('scope' => 'base', 'sizelimit' => 1, 'attributes' => array_filter(flatten(array($schema['first'], $schema['last'], $schema['full'], $schema['phone'], $schema['mobile'], $schema['email'], $schema['username'],))));
    ...
    function search($query) {
        ...
        $users = array();
        if (!$this->_bind($c)) return $users;
        $schema = static ::$schemas[$this->getSchema($c) ];
        $schema = $schema['user'];
        $r = $c->search($this->getSearchBase(), str_replace('{q}', $query, $schema['search']), array('attributes' => array_filter(flatten(array($schema['first'], $schema['last'], $schema['full'], $schema['phone'], $schema['mobile'], $schema['email'], $schema['username'], 'dn',)))));
        // XXX: Log or return some kind of error?
        if (PEAR::isError($r)) return $users;
        foreach ($r as $e) $users[] = $this->_getUserInfoArray($e, $schema);
        return $users;
    }
    function getSearchBase() {
        $base = $this->getConfig()->get('search_base');
        if (!$base && ($domain = $this->getConfig()->get('domain'))) $base = 'dc=' . str_replace('.', ',dc=', $domain);
        return $base;
    }
    ...
    function _getUserInfoArray($e, $schema) {
        ...
        if (!($first = $this->_getValue($e, $schema['first'])) || !($last = $this->_getValue($e, $schema['last']))) {
            $name = new PersonsName($this->_getValue($e, $schema['full']));
            $first = $name->getFirst();
            $last = $name->getLast();
        } else $name = "$first $last";
        return array('username' => $this->_getValue($e, $schema['username']), 'first' => $first, 'last' => $last, 'name' => $name, 'email' => $this->_getValue($e, $schema['email']), 'phone' => $this->_getValue($e, $schema['phone']), 'mobile' => $this->_getValue($e, $schema['mobile']), 'dn' => $e->dn(),);
    }
        ...
        switch ($this->type) {
            case 'staff':
                if (($user = StaffSession::lookup($username)) && $user->getId()) {
                    ...
                    $config = $this->getConfig();
                    if ($config->get('auth-staff-auto-create')) {
            ...
            case 'client':
                ...
                if ('msad' == $this->getSchema($c) && stripos($dn, ',dc=') === false) {
                    ...
                    $r = $c->search($this->getSearchBase(), sprintf('(|(userPrincipalName=%s)(samAccountName=%s))', $dn, $samid), $opts);
                    if (!PEAR::isError($r) && $r->count()) $dn = $r->current()->dn();
                }
                // Lookup all the information on the user. Try to get the email
                // addresss as well as the username when looking up the user
                // locally.
                if (!($info = $this->lookup($dn, false))) return;
                $acct = false;
                foreach (array($username, $info['username'], $info['email']) as $name) {
                    if ($name && ($acct = ClientAccount::lookupByUsername($name))) break;
                }
                if (!$acct) return new ClientCreateRequest($this, $username, $info);
                if (($client = new ClientSession(new EndUser($acct->getUser()))) && !$client->getId()) return;
                return $client;
            }
    ...
    class StaffLDAPAuthentication extends StaffAuthenticationBackend implements AuthDirectorySearch {
        static $name = /* trans */
    ...
    class ClientLDAPAuthentication extends UserAuthenticationBackend {
        ...
        function __construct($config) {
            ...
            $this->config = $config;
            if ($domain = $config->get('domain')) self::$name.= sprintf(' (%s)', $domain);
        }
        function getName() {
            $config = $this->config;
            list($__, $_N) = $config::translate();
    ...
    class LdapAuthPlugin extends Plugin {
        ...
        function bootstrap() {
            $config = $this->getConfig();
            if ($config->get('auth-staff')) StaffAuthenticationBackend::register(new StaffLDAPAuthentication($config));
            if ($config->get('auth-client')) UserAuthenticationBackend::register(new ClientLDAPAuthentication($config));
        }
    }
